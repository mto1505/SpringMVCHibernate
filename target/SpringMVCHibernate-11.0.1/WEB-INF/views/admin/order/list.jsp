...
<c:url var="NewestOrderList" value="/quan-tri/order/danh-sach?trangThai=0&page=1&limit=10" />
<c:url var="AcceptOrder" value="/quan-tri/order/accept" />
...
<body style="font-size: 16px">
	<div class="main-content">
		<div class="main-content-inner">
			...
			<a class="btn btn-primary" href="${NewestOrderList}" role="button">Đơn hàng mới nhất</a>
				<c:if test="${detailOrders!=null}">

					<div class="row justify-content-md-center">
						<div class="col-md-8">
							<h>Chi tiết đơn đặt hàng</h>
							<table class="table">
								<thead>
									<tr>
					...
					<div class="col-md-12">
									...
									<tr>
										...
										<td colspan="1">
											<div class="action-buttons">
												...
												<a
													href="${DetailOrder}?id=${dondathang.getId()}&page=${currentPage}&trangThai=${trangThai}"
										...
										<td colspan="3">${dondathang.diaChi==null ? dondathang.getKhachHang().getDiaChi():dondathang.diaChi }</td>
										...
										<td>
											<div class="btn-group">
												<div class="d.none d-sm-block action-buttons">
													...
													<a class="blue" id="acceptOrder" href="${AcceptOrder}?idDonDatHang=${dondathang.getId()}" onclick="acceptOrder(event,callback)"><i class="ace-icon fa fa-check bigger-130"></i>
													</a> <a class="green"
												...
												<div class="inline pos-rel">
													...
													<ul
														...
														<li><a href="#" class="tooltip-success"
															data-rel="tooltip" title="Edit"> <span class="green"> i<i
						...
						<form action="<c:url value='/quan-tri/order/danh-sach'/>"
							...
							<input type="hidden" value="" id="page" name="page" /> <input
								type="hidden" value="" id="maxPageItem" name="limit" />
							<!-- 	 <input type="hidden" value="" id="sortName" name="sortName" />
								 <input
									type="hidden" value="" id="sortBy" name="sortBy" />  -->
							<input type="hidden" value="${trangThai}" id="trangThai"
								name="trangThai" />
							<!-- 	<input type="hidden" value="" id="idDonHang" name="id" />					 -->
						</form>
						
						<form action="<c:url value='/quan-tri/order/danh-sach'/>"
							id="formOrderSubmit" method="POST">
							<input type="hidden" value="" id="idDonDatHang" name="page"/> 			
						</form>
						<!-- PAGE CONTENT ENDS -->
					</div>
					<!-- /.col -->
	...
	<script>
		...
		function acceptOrder(event,submitOrderForm)
		{
			event.preventDefault();
			var elementFirstParent=$(event.target).parent("a")[0];
			var linkAcceptOrder=$(elementFirstParent).attr("href");
			var $formOrderSubmit=$("#formOrderSubmit");
			var $hrefValue=$(event.target).attr("href");
			$formOrderSubmit.attr("action",linkAcceptOrder);/* +"?idDonDatHang="+id */
			console.log(linkAcceptOrder);
			submitOrderForm();
			/* $formOrderSubmit.submit(); */
		}
		$('.show-details-btn').on(
		...
		var limit = 10;//số item hiển thị lên
		$(function() {
