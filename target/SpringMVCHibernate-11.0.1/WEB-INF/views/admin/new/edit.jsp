...
<html>
    ...
    <body>
                                    ...
                                    <div class="col-sm-9">
                                        <select class="form-control" id="categoryCode" name="categoryCode">
                                           ...
                                           <c:if test="${empty model.categoryCode}">
                                            ...
                                            --%> 
                                            <c:if test="${empty model.categoryCode}">  <!-- nếu NewModel không có categoryCode (Thêm mới) -->
                                                <option value="">Chọn loại bài viết</option>
                                                <c:forEach var="item" items="${categories}">
                                                    <option value="${item.code}">${item.name}</option>
                                                </c:forEach>
        ...
        <script>
            //serializeArray trả về mảng giá trị của Form: index:{key:name,value:value} 
            $("#btnAddOrUpdateNew").click(function (e)
            {
                e.preventDefault();
                var data = {};//javascript Object
                var $formArrayDataInput = ($('#formSubmit')).serializeArray();
                $.each($formArrayDataInput, function (key, value)
                {

                    data[value.name] = value.value;
                    console.log("key.name:" + value.name, "value:" + data[value.name]);
                    console.log(data)
                })//function return ORIGINAL ARRAY
                var $id = $('#id').val(); //id của New (type: string)
                //  data['id']=$id;
            ...
            function addNew(data) {
                $.ajax({
                    url: '${APIurl}',
                    type: 'POST',
                    contentType: 'application/json', // gửi lên sever
                    data: JSON.stringify(data),
                    dataType: 'json', // từ Sever trả lại client,
