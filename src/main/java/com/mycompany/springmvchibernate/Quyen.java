 ...
 */
@Entity
@Table(name = "Quyen", catalog = "QuanLyBaoDuongDemo", schema = "dbo")
@XmlRootElement
@NamedQueries({
    @NamedQuery(name = "Quyen.findAll", query = "SELECT q FROM Quyen q"),
    @NamedQuery(name = "Quyen.findById", query = "SELECT q FROM Quyen q WHERE q.id = :id"),
...
public class Quyen implements Serializable {
    ...
    @NotNull
    @Size(min = 1, max = 30)
    @Column(name = "tenquyen", nullable = false, length = 30)
    ...
    public Quyen(Integer id) {
        this.id = id;
    }

    public Quyen(Integer id, String tenquyen) {
        this.id = id;
    ...
    public String toString() {
        return "com.mycompany.springmvchibernate.Quyen[ id=" + id + " ]";
    }
